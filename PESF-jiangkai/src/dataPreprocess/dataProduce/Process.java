...
public class Process {
	...
	private void outputLabels() {
		try {
			File labelFile = Utils.createNewFile(CommonParameters.labelsInputFilePath);
	...
	private void findDocContents() {
 		...
 		try {
 			DocWriter docWriter = new DocWriter(labels);
 		 	...
 		 	File dir = new File(CommonParameters.tokenSourcePath);
	...
	private void findDocIds() {
		try {
			...
			BufferedReader in = new BufferedReader(new FileReader(CommonParameters.labelIdTablePath));
			String line = null;
			while((line=in.readLine())!=null)
			{
				StringTokenizer itr = new StringTokenizer(line);
	...
	public void loadInLabels()
	{
		...
		try {
			BufferedReader in = new BufferedReader(new FileReader(CommonParameters.labelsSoucrceFilePath));
			String label = null;
